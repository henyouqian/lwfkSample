...
SliderGame::SliderGame():_pTouch(NULL) {
    ...
    _pSndTick = lw::Sound::create("button.wav");
...
void SliderGame::load(const char *file) {
    ...
    float textureWidth = (float)pTexture->w;
    float textureHeight = (float)pTexture->h;
...
void SliderTask::vStart() {
    lwinfo("SliderTask::vStart()");
    _pSliderGame = new SliderGame();
    glClearColor(0.f, 0.f, 0.f, 1.0f);
    
//    //collect all image names
//    std::string path = lw::File::GetReadPath().c_str();
...
//    path.append("img180.JPG");
    _pSliderGame->load("img180.JPG");
}


void SliderTask::vStop() {
    lwinfo("SliderTask::vStop()");
    delete _pSliderGame;
}
...
//    }
    ...
    lwinfo("vTouchBegan:%d", (int)(&touch));
}
...
void SliderTask::vTouchEnded(const lw::Touch &touch)  {
    ...
    lwinfo("vTouchEnded:%d", (int)(&touch));
}

void SliderTask::vTouchCanceled(const lw::Touch &touch)  {
    _pSliderGame->touchEnded(touch);
    lwinfo("vTouchCanceled:%d", (int)(&touch));
}
